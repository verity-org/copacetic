...
func TestPatchMultiPlatformImageRejectsMissingDiscoveredPlatforms(t *testing.T) {
	...
	require.Error(t, err)
	assert.EqualError(t, err, "no platforms provided for image docker.io/library/nginx:1.25")
}
...
func TestPatchMultiPlatformImageRejectsUnavailableRequestedPlatforms(t *testing.T) {
	...
	require.Error(t, err)
	assert.EqualError(t, err, "none of the specified platforms [linux/s390x] are available in the image")
}
