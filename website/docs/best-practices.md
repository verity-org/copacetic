...
:::tip Bulk Patching with Skip Detection
When using [bulk image patching](./bulk-image-patching.md#skip-already-patched-images) with `--push` and `-r`, Copa automatically implements this pattern. It checks vulnerability reports for existing patched images and skips re-patching when no new fixable vulnerabilities are found, saving time and compute in scheduled workflows.
:::
