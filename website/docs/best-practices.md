...
:::tip Bulk Patching Skip Detection + Versioning
When using [bulk image patching](./bulk-image-patching.md#skip-already-patched-images) with `--push` and `-r`, Copa checks vulnerability reports for existing patched images. It skips re-patching when no fixable vulnerabilities remain, and only creates a new versioned tag when re-patching is needed.
:::
